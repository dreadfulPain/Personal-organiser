...
**Last updated:** Session 17 (resolved the "what matters" engine for the help-me-decide build: **urgency = AI / importance = you**; **a tag is a *category*, not an importance level** — "work" a soft default you can lower, never a law (closes the coworker loophole); **milestone-pull = the third, ungameable importance signal**; mirror shows **% of time by tag**, suggest-don't-rebalance)
...
1. **Triage** — too many wants, not enough time. The app helps decide what's worth the limited time *now*, what will quietly get dropped anyway, and what's *urgent* vs what's just being over-polished. **Capacity-aware 🔒 (s15):** triage is bounded by a *limited daily budget* (cf. "spoon theory" — a fixed daily store of mental energy that, once spent, is gone). So "what matters now" must hand back a **short, finishable shortlist for today**, sized to realistic energy — *not* a ranked wall. A small set you can actually clear feeds felt-completion + the clean-slate feeling (#2); an oversized today-list just manufactures guilt (see the progress-bar guard below). **Unbounded-work frame 🔒 (s16):** some work is *never done* (the user's real case: teaching — there is always more you *could* do). For such work, "did I finish?" is the wrong question (the answer is always no). The only sane measure is **"did I spend a fair, bounded amount?"** — a *budget you set*, not a task you complete. This single frame answers *both* failure directions at once — "am I doing too little?" and "am I over-giving to people who don't deserve my spoons?" collapse into one question: **am I spending my limited budget where I value it?** Spoons applied to a whole life, not just one day.
...
- **Importance/urgency surfacing + SUGGESTED scheduling** 🟡 *(added s14; promoted to NEXT s15)* — the user asked the app to "see which tasks are more important or urgent and move them around the planner." Two halves, judged separately: **(a) surfacing** what matters most (importance vs urgency — the classic lens; directly serves the §0 triage need) = **strongly aligned, do this.** **(b) auto-moving** items around the calendar = **GUARDED HARD:** silently rearranging the user's day crosses the control pillar (§0/§1: AI *proposes*, user *decides*) and risks recreating the obligation-wall. Allowed form: the app *highlights* what's most important/urgent and *offers* a suggested plan / slots things into free gaps — **the user accepts or tweaks; nothing moves on its own.** Suggest, never impose. **(s15 — now front-runner):** real daily use put this at the **top of the queue** — after the timeline shipped, the user said seeing the laid-out day still "isn't enough… I need the next step," i.e. *help me decide.* The **surfacing** half is precisely that help. **Capacity-aware output 🔒 (s15):** what it surfaces is a **short, finishable shortlist for today** sized to a limited daily energy budget (cf. "spoon theory", §0) — small by default, **never a ranked wall**, and with **no depleting "energy meter"** (that would be the can't-fill guilt bar §0 forbids). **Optional "lighter day" control 🔵 (s15):** a later, opt-in toggle that shrinks the shortlist on low-energy days — concrete form: at most one soft prompt like *"how's today: light / normal / running low?"* (words, not numbers — friendlier for dyscalculia). Guards: always optional (never a field you must fill), never a depleting gauge, never guilt. Near-term needs no input — just keep today's set small by default. **Handle: "lens, not ledger" 🔒 (s16)** — spoons *guide* the size of today; they are never *counted, scored, or tallied*. Counting = admin + a guilt meter, the two things the app exists to remove.
  - **THE "WHAT MATTERS" ENGINE — resolved 🔒 (s17).** Three inputs feed the shortlist, each handled by whoever can actually judge it:
    - **Urgency = the AI's job.** Urgency lives in the clock (deadline? how close? real or just pressure?). The app knows today's date, so the AI reads it reliably; the only user touch is a one-tap confirm that a deadline is *real* vs imposed.
    - **Importance = the user's job.** Importance = what the user *values*; the AI cannot see values, only guess from words. So importance stays user-steered (one-tap "this matters", or inherited from a goal they care about); the AI's guess is a starting position, never the verdict.
    - **A TAG IS A *CATEGORY*, NOT AN IMPORTANCE LEVEL 🔒.** Critical, from the user's real wound (a coworker "was good at making issues work-related so I couldn't escape"). If "work tag ⇒ important" were baked in, anyone who labels a demand "work" makes it important *in the user's own system* — her trap, automated. So: tags = category/grouping only (great for the mirror's percentages); whether a work task *matters* is a separate, user-owned call. "Work" may **default** to higher importance (honoring the user's value) but as a lowerable starting position, **never a law.** This gives back the language she blurred: *"yes it's work — no it's not important."* (Restates & strengthens the s15 "tags are not the importance formula" rule.)
    - **Milestone-pull = the third signal 🔒, and the cleanest.** "This task would *complete* a milestone (§9)" is a strong importance signal AND — unlike a tag — **it's the user's own and cannot be faked** (no one else can manufacture your milestones). Real importance surfaces as "moves me toward something I chose to finish." Also the antidote to never-done work: it wrings a *real finish* out of endless jobs. **Guard:** one weighted input, NOT an override — a near-done milestone must not jump ahead of a hard deadline today.
    - **Through-line / the protection:** labels and other people's framing never decide what matters — the user's values (importance) and goals (milestones) do. Two axes kept separate (urgent ≠ important; category ≠ importance) is itself the defense against manufactured urgency.
...
- **The mirror, reshaped 🔒 (s16):** the user wants Looking-back to grow into seeing **where time & attention went, by category** (work, rest, personal, etc.) over weeks — so *they* can spot drift in either direction (too little on what they value; too much given to unreasonable demands). Hard rules: it **describes, never judges** — shows the shape of the time only when asked, never nags, never scores, never a "you fell short" verdict. *Name the load, not the person* applies to the **self** too: the mirror may show the shape of your time, never tell you you're not enough. ("I spent 50% on manga → I'm bad" is the guilt-machine to design *against*.) It is a mirror for **your own steering**, NOT a scorecard for anyone else.
  - **% of time by tag 🔒 (s17):** the mirror's category breakdown is computed from **tags** (§0.2) — "X% of your time went to [tag]" over weeks. **"Sort my time to compensate" — guarded:** the app may *show* an imbalance and *suggest* ("almost no personal time this month — want to protect some?"), but **never silently re-sorts your life.** The right balance is the user's to set; surface + suggest, accept-or-ignore (control pillar). Still describes, never judges.
- **Time ≠ results 🔒 (s16):** the user's own sharp catch — the app can only see *time/attention spent*, never *results/quality* (their work was graded on outcomes). So the app **cannot be a referee** that proves "I pulled my weight"; built to settle that argument, it would only lose. Consequence: a time-mirror is for self-knowledge, not proof. To get nearer to *results*, let the app also hold **wins — what was actually achieved — logged SEPARATELY from time** (this is the *attached-artifacts / evidence* shape §0.2 + felt-completion); wins are closer to how real worth is measured than any clock.
...
- **Milestone-pull feeds triage 🔒 (s17):** a task that would *complete* (or nearly complete) the current milestone gets a priority boost in the help-me-decide shortlist (§0.2) — it's the third "what matters" input, and the only one that is **the user's own and ungameable.** Guard: a weighted input, never an override of a hard deadline. Depends on milestone placement being right (see caveat above).
...
- **Session 17:** **Resolved the "what matters" engine** for the help-me-decide build (answers the s16 open fork *AI-leads vs user-steers*). It's **split by signal, not all-or-nothing:** **urgency = the AI** (it's in the clock; AI reads it, user just confirms a deadline is real), **importance = the user** (the AI can't see values; its guess is only a starting position). The user proposed "work tag ⇒ important"; flagged the trap *they themselves named* (a coworker who "made issues work-related so I couldn't escape") → locked **a tag is a category, NOT an importance level**: tags = grouping only (and the mirror's % breakdown), "work" may be a *soft default* you can lower but **never a law** — otherwise her trap is automated. Added **milestone-pull as the third importance signal** — strong, and the only ungameable one (your milestones can't be faked by others); also the antidote to never-done work; weighted input, not an override. Mirror gains **% of time by tag**, with "compensate" guarded to **suggest-don't-rebalance**. Through-line: *labels and other people's framing never decide importance — your values and goals do.* Still a design session; no code — feeds straight into the NEXT build's signal-prep + surfacing. *(Source: user reflection on a past-workplace conflict; reduced to existing shapes — generality check passed.)*
- **Session 16:** **Design conversation, no build.** Working from the user's real ex-job (teaching) and a workplace conflict where someone judged them as not pulling their weight. Several locks: **(1) Unbounded-work frame (§0)** — never-ending jobs measured as a *budget spent*, not a finish line; one frame that answers both "too little?" and "too much for the wrong people?" → *am I spending my limited budget where I value it?* **(2) The mirror reshaped (§5)** — Looking-back grows into *where time/attention went by category, over weeks*, that the user **chooses** to look at, to spot drift either way; strict guard: **describes, never judges**, no nag, no score, no "you fell short"; *name the load, not the person* extended to the self. **(3) Time ≠ results (§5)** — the user's own incisive point: the app sees only time, not outcomes (their work was graded on results), so it **cannot be a referee** to prove their worth — building it for that would lose; instead log **wins separately from time** (evidence shape §0.2) as the nearer-to-results record. The credibility hurt is real but *not* something a time-log can resolve; reframed from "settle the past / prove it to others" → "self-knowledge to steer my own future" (the healthy, buildable version). All of this is **later v2 reflection layer** — does NOT change the immediate next build (the "help me decide" shortlist, §0.2/NEXT). *(Source: user reflection on teaching's never-done nature + a past-workplace credibility conflict; reduced cleanly to existing shapes — generality check passed.)*
...
**Recommended next build → the "what matters now" layer (triage; §0.2, §0 need #1).** Two small steps:
  1. **Prep the signals first (s15 plan, refined s17):** have the AI also output — and the app store — an **importance** level (matters a lot / normal / minor) — but per the s17 split, the AI only *proposes* a starting guess and **the user owns it** (one-tap adjust); free-form **tags** (labels for *context + grouping* only, and the mirror's % breakdown — NEVER hard-coded meanings, NEVER "work tag ⇒ important"; a tag is a *category*, not an importance level, §0.2); a **deadline vs set-time** flag (sharpens AI-judged urgency); and **milestone-pull** (does this complete/near-complete a milestone, §9 — the ungameable importance signal). All filled by the AI *during the sort* — no new fields to fill — and shown lightly in the zones so you start *seeing* important/urgent.
  2. **Then the surfacing:** the app *highlights* what matters — combining **AI-judged urgency + user-owned importance + milestone-pull** — and hands back a **short, finishable shortlist for today** — capacity-aware, small by default (§0 spoons), never a ranked wall, never a depleting meter. It **suggests**; you decide; **nothing auto-moves** (control pillar §0/§1).
