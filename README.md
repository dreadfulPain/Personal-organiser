...
## Speaking instead of typing — not yet

Voice input is **switched off**, and no microphone appears anywhere in the app.

The groundwork is in place for an **in-house** version: point `STT_URL` in `.env`
at your own local Whisper server and dictation turns on, with the audio
transcribed on this computer and never leaving it — the same arrangement as the
AI. There is deliberately no cloud speech option; browser dictation would send
your recordings to Chrome's or Edge's servers, which is not what this app is for.
